...
function getDailyAnimatronic() {
  const now = new Date();
  const start = new Date(now.getFullYear(), 0, 0);
  const day = Math.floor((now - start) / 86400000);
  return ANIMATRONICS[day % ANIMATRONICS.length];
}
...
const POWER_STAGES = [
  ...
  { time:"1 AM",  pct:80, color:"#dd2200" },
  { time:"2 AM",  pct:60, color:"#ffdd00" },
  { time:"3 AM",  pct:35, color:"#ff8800" },
  { time:"4 AM",  pct:15, color:"#ff3300" },
...
function TypeBadge({ label }) {
  ...
  return (
    <span style={{display:"inline-flex",alignItems:"center",padding:"3px 10px",borderRadius:"4px",background:`${color}22`,border:`1px solid ${color}55`,fontFamily:"'Courier New',monospace",fontSize:"0.6rem",color,letterSpacing:"0.08em",whiteSpace:"nowrap"}}>{label}</span>
  );
}

// CRT monitor sitting on the desk
  ...
  return (
    <div style={{width:"100%",position:"relative",userSelect:"none"}}>
      ...
      <div style={{
        ...
        padding:"12px 12px 0",
        boxShadow:"0 0 0 2px #111, 0 8px 40px rgba(0,0,0,0.9), inset 0 1px 0 rgba(255,255,255,0.05)",
        position:"relative",
      }}>
        {/* Monitor brand */}
        ...
        <div style={{background:"#0a0a0a",borderRadius:"6px 6px 0 0",padding:"8px",boxShadow:"inset 0 0 20px rgba(0,0,0,0.8)"}}>
          ...
          <div style={{
            ...
            overflow:"hidden",
            aspectRatio:"4/3",
            background: jumpscaring ? "#ff0000" : "#000",
            position:"relative",
            ...
            {jumpscaring ? (
              <div style={{zIndex:11,textAlign:"center",padding:"12px",animation:"jumpIn 0.08s ease both"}}>
                ...
                <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:"clamp(0.9rem,2vw,1.2rem)",color:"#fff",letterSpacing:"0.1em",marginTop:"8px",textShadow:"0 0 20px #fff"}}>
                  IT WAS {answer.name.toUpperCase()}
                </div>
              </div>
            ) : poweredDown ? (
              <div style={{zIndex:11,textAlign:"center",padding:"16px",animation:"fadeIn 0.5s ease"}}>
                ...
                <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:"clamp(0.85rem,2vw,1.1rem)",color:"rgba(200,0,0,0.6)",letterSpacing:"0.12em",marginBottom:"4px"}}>
                  {answer.name.toUpperCase()}
                </div>
                <div style={{fontFamily:"'Courier New',monospace",fontSize:"0.5rem",color:"rgba(200,0,0,0.3)",letterSpacing:"0.1em"}}>POWERED DOWN</div>
              </div>
            ) : showSilhouette ? (
              <div style={{zIndex:11,textAlign:"center",padding:"16px",animation:"fadeIn 0.5s ease"}}>
                <div style={{
                  width:"clamp(60px,12vw,90px)",height:"clamp(80px,16vw,120px)",
                  background:"rgba(200,0,0,0.06)",
                  border:"1px solid rgba(124,252,0,0.15)",
                  borderRadius:"6px",margin:"0 auto 10px",
                  display:"flex",alignItems:"center",justifyContent:"center",
                  fontSize:"clamp(2rem,5vw,3rem)",filter:"brightness(0.15)",
                }}>🤖</div>
                <div style={{fontFamily:"'Courier New',monospace",fontSize:"0.52rem",color:"rgba(200,0,0,0.4)",letterSpacing:"0.12em"}}>
                  SILHOUETTE DETECTED
                </div>
              </div>
            ) : isOver ? (
              <div style={{zIndex:11,textAlign:"center",padding:"16px"}}>
                {answer.image
                  ? <img src={answer.image} alt={answer.name} style={{maxHeight:"140px",objectFit:"contain",animation:"fadeIn 0.5s ease"}}/>
                  : <div style={{fontSize:"clamp(2rem,5vw,3.5rem)",marginBottom:"8px",animation:"fadeIn 0.5s ease"}}>🤖</div>
                }
                <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:"clamp(0.85rem,2vw,1.1rem)",color:gameState==="won"?"#cc0000":"#ff3300",letterSpacing:"0.12em",textShadow:gameState==="won"?"0 0 16px #cc000088":"0 0 16px #ff330088"}}>
                  {answer.name.toUpperCase()}
                </div>
              </div>
            ) : (
              ...
              <div style={{zIndex:11,textAlign:"center",padding:"20px"}}>
                <div style={{width:"clamp(50px,10vw,70px)",height:"clamp(50px,10vw,70px)",margin:"0 auto 10px",border:"1px solid rgba(124,252,0,0.15)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
                ...
                <div style={{fontFamily:"'Courier New',monospace",fontSize:"0.52rem",color:"rgba(200,0,0,0.25)",letterSpacing:"0.14em"}}>ENTITY UNIDENTIFIED</div>
                {guessCount < 5 && <div style={{fontFamily:"'Courier New',monospace",fontSize:"0.45rem",color:"rgba(200,0,0,0.15)",letterSpacing:"0.1em",marginTop:"4px"}}>SILHOUETTE AT {5-guessCount} MORE WRONG</div>}
              </div>
            )}

            {/* Camera overlay text */}
            <div style={{position:"absolute",top:"6px",left:"8px",zIndex:12,fontFamily:"'Courier New',monospace",fontSize:"0.45rem",color:"rgba(200,0,0,0.4)",letterSpacing:"0.12em"}}>CAM 1A</div>
            <div style={{position:"absolute",top:"6px",right:"8px",zIndex:12,fontFamily:"'Courier New',monospace",fontSize:"0.45rem",color:"rgba(200,0,0,0.4)",letterSpacing:"0.12em"}}>
        ...
        {/* Monitor bottom strip with power */}
      ...
      {/* Monitor stand */}
      <div style={{width:"30%",height:"16px",background:"linear-gradient(180deg,#1e1e1e,#141414)",margin:"0 auto",boxShadow:"inset 0 -1px 0 #111"}}/>
...
// Big animatronic file card per guess
function GuessCard({ guessed, answer, index }) {
  const gm = guessed.game      === answer.game;
  const tm = guessed.type      === answer.type;
  const lm = guessed.location  === answer.location;
  const nm = guessed.activeFrom=== answer.activeFrom;
  const bm = guessed.behaviour === answer.behaviour;
  const matchCount = [gm,tm,lm,nm,bm].filter(Boolean).length;

  const Attr = ({ label, value, match, wide }) => (
    <div style={{
      flex:wide?2:1,
      background:match?"rgba(200,0,0,0.07)":"rgba(255,50,50,0.06)",
      border:`1px solid ${match?"rgba(200,0,0,0.25)":"rgba(255,50,50,0.18)"}`,
      borderRadius:"6px",
      padding:"10px 10px",
      minWidth:0,
    }}>
      <div style={{fontFamily:"'Courier New',monospace",fontSize:"0.52rem",color:match?"rgba(200,0,0,0.5)":"rgba(255,80,80,0.5)",letterSpacing:"0.14em",marginBottom:"5px",textTransform:"uppercase"}}>{label}</div>
      <div style={{fontSize:"0.75rem",color:match?"#cc0000":"#ff5050",fontWeight:"bold",fontFamily:"'Courier New',monospace",lineHeight:1.3,wordBreak:"break-word"}}>
        {value}
      </div>
  ...
  return (
    <div style={{
      background:"#0c0c0c",
      border:`1px solid ${matchCount===5?"rgba(200,0,0,0.4)":"#1e1e1e"}`,
      borderRadius:"10px",
      overflow:"hidden",
      animation:"cardSlideIn 0.4s cubic-bezier(0.34,1.2,0.64,1) both",
      boxShadow:matchCount===5?"0 0 20px rgba(200,0,0,0.12)":"0 2px 12px rgba(0,0,0,0.6)",
      flexShrink:0,
    }}>
      {/* Card header */}
      <div style={{
        background:matchCount===5?"rgba(200,0,0,0.1)":"#111",
        ...
        <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
          ...
          <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:"1rem",color:matchCount===5?"#cc0000":"#ccc",letterSpacing:"0.08em",textShadow:matchCount===5?"0 0 12px rgba(200,0,0,0.5)":"none"}}>
            {guessed.name.toUpperCase()}
          </div>
          <TypeBadge label={guessed.type}/>
        </div>
        <div style={{fontFamily:"'Courier New',monospace",fontSize:"0.6rem",color:matchCount===5?"#cc0000":matchCount>=3?"#ffb300":"#ff5050",letterSpacing:"0.08em",fontWeight:"bold"}}>
          {matchCount}/5
        </div>
      </div>
      {/* Attribute grid */}
...
function Picker({ onSelect, usedNames }) {
  ...
  const available = ANIMATRONICS.filter(a =>
    !usedNames.includes(a.name) &&
  ...
  return (
    <div style={{position:"relative",flex:1}}>
      <div onClick={()=>setOpen(o=>!o)} style={{
        ...
        border:`1px solid ${open?"rgba(200,0,0,0.35)":"#1e1e1e"}`,
        background:"#0c0c0c",color:"rgba(200,0,0,0.6)",
        fontFamily:"'Courier New',monospace",fontSize:"0.82rem",
        letterSpacing:"0.08em",cursor:"pointer",
        display:"flex",alignItems:"center",justifyContent:"space-between",
        userSelect:"none",
        boxShadow:open?"0 0 20px rgba(124,252,0,0.08)":"0 2px 8px rgba(0,0,0,0.5)",
      }}>
      ...
      {open && (
        <div style={{
          ...
          zIndex:100,maxHeight:"340px",
          display:"flex",flexDirection:"column",
        }}>
          <div style={{padding:"10px",borderBottom:"1px solid #1a1a1a"}}>
            <input value={search} onChange={e=>setSearch(e.target.value)}
              placeholder="Search animatronic name..." autoFocus
              style={{width:"100%",background:"#050505",border:"1px solid #1e1e1e",borderRadius:"6px",padding:"9px 12px",color:"#cc0000",fontFamily:"'Courier New',monospace",fontSize:"0.75rem",outline:"none"}}
            />
          </div>
          <div style={{overflowY:"auto",flex:1}}>
            {Object.entries(grouped).map(([game,chars])=>(
              <div key={game}>
                <div style={{padding:"7px 14px",background:"#0a0a0a",fontFamily:"'Courier New',monospace",fontSize:"0.52rem",color:"rgba(200,0,0,0.35)",letterSpacing:"0.2em",borderBottom:"1px solid #141414",borderTop:"1px solid #141414"}}>
                  — {game.toUpperCase()} —
                </div>
                {chars.map(a=>(
                  <div key={a.id}
                    onClick={()=>{onSelect(a);setOpen(false);setSearch("");}}
                    style={{padding:"11px 14px",borderBottom:"1px solid #111",cursor:"pointer",display:"flex",alignItems:"center",gap:"12px",transition:"background 0.12s"}}
                    onMouseEnter={e=>e.currentTarget.style.background="#0f0f0f"}
                    onMouseLeave={e=>e.currentTarget.style.background="transparent"}
                  >
                    <div style={{width:"6px",height:"6px",borderRadius:"50%",background:"rgba(200,0,0,0.3)",flexShrink:0}}/>
                    <div style={{flex:1}}>
                      ...
                      <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                        ...
                        <span style={{fontFamily:"'Courier New',monospace",fontSize:"0.5rem",color:"#444",letterSpacing:"0.06em"}}>{a.activeFrom}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {available.length===0 && (
              <div style={{padding:"20px",textAlign:"center",color:"#333",fontFamily:"'Courier New',monospace",fontSize:"0.7rem"}}>NO RESULTS FOUND</div>
            )}
          </div>
        </div>
      )}
...
function ResultModal({ answer, guesses, gameState, onClose }) {
  ...
  const copy = () => navigator.clipboard.writeText(
    ["WEIRDORDLE — Animatronidle 🐻",
     won?`Survived! Identified in ${guesses.length}/${MAX}`:"It got me...",
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,0.93)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",animation:"fadeIn 0.3s ease"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#0a0a0a",border:`2px solid ${won?"rgba(200,0,0,0.3)":"rgba(255,50,50,0.3)"}`,borderRadius:"12px",padding:"28px",maxWidth:"440px",width:"100%",fontFamily:"'Courier New',monospace",boxShadow:`0 0 60px ${won?"rgba(200,0,0,0.08)":"rgba(255,0,0,0.12)"}`,animation:"popIn 0.5s cubic-bezier(0.34,1.56,0.64,1) both"}}>
        <div style={{textAlign:"center",marginBottom:"22px"}}>
          ...
          <div style={{fontSize:"0.65rem",color:"rgba(200,0,0,0.4)",letterSpacing:"0.14em",marginBottom:"4px"}}>THE ANIMATRONIC WAS</div>
        ...
        <div style={{background:"#0d0d0d",border:"1px solid #1e1e1e",borderRadius:"8px",padding:"14px",marginBottom:"18px"}}>
          <div style={{fontSize:"0.52rem",color:"rgba(200,0,0,0.35)",letterSpacing:"0.18em",marginBottom:"10px"}}>ANIMATRONIC FILE</div>
        ...
        <div style={{display:"flex",gap:"8px"}}>
          <button onClick={copy} style={{flex:1,padding:"11px",borderRadius:"8px",border:"1px solid rgba(124,252,0,0.2)",background:"transparent",color:"#cc0000",fontFamily:"'Courier New',monospace",fontSize:"0.68rem",letterSpacing:"0.1em",cursor:"pointer"}}>📋 COPY RESULT</button>
          <button onClick={onClose} style={{flex:1,padding:"11px",borderRadius:"8px",border:"1px solid #1e1e1e",background:"transparent",color:"#555",fontFamily:"'Courier New',monospace",fontSize:"0.68rem",letterSpacing:"0.1em",cursor:"pointer"}}>BACK TO OFFICE</button>
        </div>
        <div style={{textAlign:"center",marginTop:"12px",fontSize:"0.5rem",color:"#222",letterSpacing:"0.12em"}}>NEW ANIMATRONIC AT MIDNIGHT · WEIRDORDLE.COM</div>
      </div>
    </div>
  );
}
...
export default function App() {
  const ANSWER = getDailyAnimatronic();
    ...
    <div style={{height:"100vh",width:"100vw",overflow:"hidden",background:"#060606",color:"#ccc",fontFamily:"'Courier New',monospace",display:"flex",flexDirection:"column",boxSizing:"border-box"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes jumpIn{0%{transform:scale(0.3) rotate(-5deg);opacity:0}60%{transform:scale(1.1) rotate(1deg)}100%{transform:scale(1) rotate(0deg);opacity:1}}
      ...
      {/* Subtle scanline across whole page */}
        ...
        <div onClick={()=>setShowHelp(false)} style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.92)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",animation:"fadeIn 0.2s ease"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#0a0a0a",border:"1px solid #1e1e1e",borderRadius:"12px",padding:"26px",maxWidth:"400px",width:"100%",position:"relative"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"18px"}}>
              ...
              <div>
                <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:"1.2rem",color:"#cc0000",letterSpacing:"0.1em"}}>HOW TO PLAY</div>
                <div style={{fontSize:"0.52rem",color:"#333",letterSpacing:"0.14em"}}>ANIMATRONIDLE</div>
              </div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:"13px"}}>
              {[
                ["🖥️","Check the cameras","Something is in the building. Watch the monitor and figure out which animatronic it is."],
                ["🎮","Select an animatronic","Use the picker below to choose who you think it is, grouped by game."],
                ["🟩","Attribute matching","Game, Location, Active From and Behaviour all turn green when they match the answer."],
                ["⚡","Power drains","Each wrong guess moves the clock forward and drains power. At 4% the silhouette appears."],
                ["😱","Wrong = jumpscare","Use all 6 guesses wrong and it gets you. Guess right and it powers down safely."],
                ["📋","Share","Copy your result after the game. New animatronic every night at midnight."],
              ].map(([icon,title,desc])=>(
      ...
      <div style={{
        ...
        gap:"0",
        minHeight:0,
        background:"radial-gradient(ellipse at 20% 50%,#0d0d0d 0%,#060606 100%)",
      }}>

        {/* LEFT PANEL — the desk with monitor on it */}
        <div style={{
          display:"flex",flexDirection:"column",
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"14px",flexShrink:0}}>
            <a href="/" style={{fontSize:"0.75rem",letterSpacing:"0.06em",color:"#333",cursor:"pointer",textDecoration:"none",transition:"color 0.2s"}}
            ...
            <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
              <div style={{fontSize:"0.48rem",letterSpacing:"0.14em",color:"#000",background:"#cc0000",padding:"2px 8px",borderRadius:"20px",fontWeight:"bold"}}>DAILY</div>
          ...
          <div style={{
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div style={{fontSize:"0.55rem",color:"#2a2a2a",letterSpacing:"0.14em"}}>
                FREDDY FAZBEAR'S PIZZA · NIGHT GUARD STATION
              </div>
              <div style={{display:"flex",gap:"5px"}}>
                {Array.from({length:MAX}).map((_,i)=>(
                  <div key={i} style={{
                    width:"12px",height:"12px",borderRadius:"2px",
                    background:i<guesses.length?(guesses[i].name===ANSWER.name?"#cc0000":"#ff3300"):i===guesses.length&&state==="playing"?"#222":"#111",
                    border:`1px solid ${i<guesses.length?(guesses[i].name===ANSWER.name?"rgba(200,0,0,0.4)":"rgba(255,50,0,0.3)"):"#1a1a1a"}`,
                    transition:"all 0.3s",
                  }}/>
                ))}
              </div>
            ...
            {/* Fan decoration */}
            <div style={{display:"flex",alignItems:"center",gap:"6px",opacity:0.15}}>
              {Array.from({length:20}).map((_,i)=>(
                <div key={i} style={{flex:1,height:"1px",background:"#444"}}/>
              ))}
            </div>
          </div>
        </div>

        {/* RIGHT PANEL — guess log */}
        <div style={{
          display:"flex",flexDirection:"column",
          background:"#080808",
          minHeight:0,
        ...
        <div style={{
          ...
          background:"linear-gradient(transparent,#060606 30%)",
        }}>
          {state==="playing" ? (
            <div style={{display:"flex",gap:"8px"}}>
            ...
            <div style={{display:"flex",gap:"8px"}}>
              <button onClick={()=>setShowResult(true)} style={{flex:2,padding:"13px",borderRadius:"8px",border:"1px solid rgba(124,252,0,0.2)",background:"rgba(200,0,0,0.05)",color:"#cc0000",fontFamily:"'Courier New',monospace",fontSize:"0.75rem",letterSpacing:"0.1em",cursor:"pointer"}}>
                🐻 {state==="won"?"VIEW RESULT":"SEE WHO IT WAS"}
              </button>
              <a href="/" style={{flex:1,padding:"13px",borderRadius:"8px",border:"1px solid #1e1e1e",background:"transparent",color:"#333",fontFamily:"'Courier New',monospace",fontSize:"0.75rem",letterSpacing:"0.1em",cursor:"pointer",textDecoration:"none",display:"flex",alignItems:"center",justifyContent:"center"}}>
                🏠 HOME
              </a>
            </div>
          )}
